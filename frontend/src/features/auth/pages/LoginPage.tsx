...
import { Mail, Lock, Wind, AlertCircle } from 'lucide-react';
...
export const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-base)] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Glow orbs */}
      <div className="absolute -top-48 -left-48 w-[600px] h-[600px] rounded-full bg-[radial-gradient(circle,rgba(56,189,248,0.08),transparent_70%)] pointer-events-none" />
      <div className="absolute -bottom-36 -right-36 w-[500px] h-[500px] rounded-full bg-[radial-gradient(circle,rgba(99,102,241,0.08),transparent_70%)] pointer-events-none" />
      ...
      <div className="relative w-full max-w-md bg-[var(--color-bg-surface)] border border-[var(--color-border)] rounded-2xl p-10 shadow-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-[var(--color-text-primary)]">Welcome Back</h1>
          <p className="text-xs tracking-widest text-[var(--color-text-muted)] uppercase mt-1">Log in to Vortex-Gen</p>
        </div>
        ...
        <p className="text-center mt-6 text-sm text-[var(--color-text-muted)]">
          ...
          <Link to="/signup" className="text-[var(--color-accent)] font-semibold hover:underline">Sign up</Link>
        </p>
      </div>
    </div>
  );
