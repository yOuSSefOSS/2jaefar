...
const FlowParticles = ({ isActive, windSpeed, pitchAngle, airfoilPts }) => {
  ...
  const flowVisualMode = 'streaklines';
...
const EducationalWindTunnel = ({
  ...
  const turntableActive = false;
  const snapRef = useRef(null);
  const handleSnap = useCallback((v)=>{ if(snapRef.current) snapRef.current(v); },[]);

  const hasShape = !!airfoilPoints;
  const displayPitch = pitchAngle;
  const airfoilPts = airfoilPoints;
  ...
  return (
    <div className="relative w-full h-full min-h-[400px] glass-panel flex flex-col items-center justify-center overflow-hidden" style={{willChange:'transform',transform:'translateZ(0)'}}>
      {/* Stars and Nebulae Background */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#020617] to-[#0f172a] z-0 overflow-hidden pointer-events-none">
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-50 animate-[pulse_6s_ease-in-out_infinite]" />
        <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-sky-500/20 blur-[120px] rounded-full animate-[spin_40s_linear_infinite]" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-indigo-500/20 blur-[120px] rounded-full animate-[spin_50s_linear_infinite_reverse]" />
      </div>

      <ViewportGizmo cameraStr={cameraStr} onSnapView={handleSnap}/>
      <AoAGauge pitchAngle={displayPitch} positiveStallAngle={positiveStallAngle} negativeStallAngle={negativeStallAngle} />
      ...
      <div className="absolute inset-0 z-0 cursor-crosshair">
        <Canvas>
          ...
          <group rotation={[-Math.PI/2,0,0]}>
            ...
            {airfoilPts && (
              <AirfoilMesh points={airfoilPts} showHeatmap={showHeatmap} isSimulating={isSimulating} pitchAngle={displayPitch} />
            )}
            {airfoilPts && (
...
export default EducationalWindTunnel;
