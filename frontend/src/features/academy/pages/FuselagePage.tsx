...
import { Box, ChevronRight, Users, Package, Shield, Layers, ArrowRight } from 'lucide-react';
...
const QUIZ = {
  question: 'Why are airplane windows round instead of square?',
  options: [
    { text: 'They look better aesthetically', correct: false },
    { text: 'Round shapes distribute pressure stress evenly, preventing cracks', correct: true },
    { text: 'It\'s cheaper to manufacture round windows', correct: false },
    { text: 'Square windows would block the view', correct: false },
  ],
  explanation: 'Round (oval) windows distribute stress evenly around their perimeter. Square corners create stress concentrations that can lead to fatigue cracks — as tragically demonstrated by the de Havilland Comet crashes in 1954.',
};
...
const FuselageSection = () => {
  ...
  return (
    <div className="min-h-full px-6 lg:px-10 py-8 max-w-4xl mx-auto">
      ...
      >
        <div className="text-[10px] font-mono tracking-widest text-[var(--color-edu-amber)] uppercase mb-3 flex items-center gap-2">
          ✦ Knowledge Check
        </div>
        ...
        {showExplanation && (
          ...
          >
            <span className="font-bold text-[var(--color-edu-green)]">Explanation: </span>
            {QUIZ.explanation}
          </motion.div>
        )}
      </motion.div>
      ...
      >
        <Link to="/explore" className="text-sm font-semibold text-[var(--color-edu-text-muted)] hover:text-white transition-colors flex items-center gap-2">
          ← Back to Explorer
        </Link>
