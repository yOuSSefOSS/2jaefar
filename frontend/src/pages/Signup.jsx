...
const Signup = () => {
  ...
  const inp = (extra = {}) => ({
    ...
    paddingLeft: '2.5rem', paddingRight: '2.8rem',
  ...
  const lbl = { display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '0.4rem', letterSpacing: '0.08em', textTransform: 'uppercase' };
  const eyeBtn = { position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#475569', padding: 0 };
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #020817 0%, #0a0f1e 50%, #020817 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', fontFamily: "'Inter', 'Segoe UI', sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: '440px', background: 'linear-gradient(145deg, rgba(15,23,42,0.97), rgba(8,15,32,0.99))', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '20px', padding: '2.5rem', boxShadow: '0 25px 80px rgba(0,0,0,0.6)', backdropFilter: 'blur(20px)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
          ...
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0, background: 'linear-gradient(135deg, #e0f2fe, #818cf8, #7dd3fc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Create Account</h1>
          <p style={{ color: '#475569', margin: '0.4rem 0 0', fontSize: '0.875rem', letterSpacing: '0.05em' }}>JOIN VORTEX-GEN TODAY</p>
        </div>

        {error && (
          <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '1.25rem', display: 'flex', alignItems: 'flex-start', gap: '0.5rem', color: '#f87171', fontSize: '0.85rem' }}>
        ...
        <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required minLength={6} style={inp()}
                onFocus={e => e.target.style.borderColor = 'rgba(99,102,241,0.6)'}
                onBlur={e => e.target.style.borderColor = 'rgba(51,65,85,0.8)'} />
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type={showConfirm ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="••••••••" required
                style={inp({ borderColor: !passwordsMatch ? 'rgba(239,68,68,0.5)' : 'rgba(51,65,85,0.8)' })}
                onFocus={e => e.target.style.borderColor = !passwordsMatch ? 'rgba(239,68,68,0.5)' : 'rgba(99,102,241,0.6)'}
                onBlur={e => e.target.style.borderColor = !passwordsMatch ? 'rgba(239,68,68,0.4)' : 'rgba(51,65,85,0.8)'} />
              <button type="button" onClick={() => setShowConfirm(v => !v)} style={eyeBtn}>
                {showConfirm ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>
            {!passwordsMatch && <p style={{ color: '#f87171', fontSize: '0.73rem', marginTop: '0.3rem' }}>Passwords do not match</p>}
          </div>

          <button type="submit" disabled={loading} style={{ width: '100%', padding: '0.8rem', marginTop: '0.25rem', background: loading ? 'rgba(99,102,241,0.3)' : 'linear-gradient(135deg, #6366f1, #0ea5e9)', border: 'none', borderRadius: '10px', color: 'white', fontSize: '0.9rem', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', transition: 'all 0.2s', letterSpacing: '0.05em', boxShadow: loading ? 'none' : '0 4px 20px rgba(99,102,241,0.35)' }}>
        ...
        <button onClick={handleGoogleSignup} disabled={googleLoading} style={{ width: '100%', padding: '0.75rem', background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(51,65,85,0.8)', borderRadius: '10px', color: '#e2e8f0', fontSize: '0.9rem', fontWeight: 600, cursor: googleLoading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', transition: 'all 0.2s' }}
      ...
      <style>{`@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } } input::placeholder { color: #334155; } input:-webkit-autofill { -webkit-box-shadow: 0 0 0 100px #020817 inset !important; -webkit-text-fill-color: #e2e8f0 !important; }`}</style>
    </div>
  );
};
